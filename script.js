...
function renderNavLogoGalleryPage(page = 1, skipHistory = false) {
    ...
    pageItems.forEach(item => {
        ...
        html += `
            <div class="gallery-item" style="border:1px solid #333;border-radius:8px;overflow:hidden;background:#252538;">
                <img src="${image}" alt="${item.nav_title}" style="width:100%;height:150px;object-fit:cover;" />
                <div class="gallery-info" style="padding:0.8rem;">
                    <strong>${item.nav_title}</strong>
                    <p style="font-size:0.9rem;color:#ccc;">${shortDesc}</p>
                    <button style="margin-top:5px;padding:0.4rem 0.8rem;background:#ffb300;border:none;border-radius:5px;cursor:pointer;color:#1a1a28;" onclick="openGalleryFromNavLogo(${item.id})">View</button>
                </div>
            </div>
        `;
    });
    ...
    for (let i = 1; i <= totalPages; i++) {
        paginationHTML += `<button class="${i === page ? "active" : ""}" onclick="changeNavLogoPage(${i})" style="margin:0 4px;padding:4px 8px;cursor:pointer;">${i}</button>`;
    }
    paginationContainer.innerHTML = paginationHTML;

    // Update URL if not skipping history
    if (!skipHistory) {
        const baseUrl = `${window.location.origin}${window.location.pathname}`;
...
function openNavLogoOverviewModal(skipHistory = false, page = 1) {
    ...
    // Now render page items
    renderNavLogoGalleryPage(page, skipHistory);
}

// ---------- Handle direct URL (for ?gallery=navlogo&page=N) ----------
function handleDirectGalleryLinks(data) {
    ...
    setTimeout(() => openOverviewModal(gallery.id, true), 500);
}
